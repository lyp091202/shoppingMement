...
* @param {Object} 封装的对象
* 用法:
*  var opionts = { url: "/xxx/aaa/", data: { param1: "xx", param2: "cc"}, fnSuccess: succFnName, fnFaild: faildFnName };
*  sendUrl(opionts);
*/
utils.sendUrl = function (options) {
    $.ajax({
        url: options.url || "",
        ...
        success: function (result, textStatus, xmlHttpRequest) {
            if (options.fnSuccess) {
                options.fnSuccess(result);
            }
        },
        error: function (xmlHttpRequest, textStatus, errorThrown) {
            if (options.fnFaild) {
                options.fnFaild(errorThrown);
            }
        }
    });
}
